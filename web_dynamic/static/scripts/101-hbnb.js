  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      data.forEach((p) => {
        ...
        article.append(div2);
        const div3 = $('<div></div>').addClass('description').html(p.description);
        article.append(div3);
        const div4 = $('<div></div>').addClass('reviews');
        div4.append($(`<h2>Reviews<span data-id="${p.id}">show</span></h2>`));
        div4.append($('<ul></ul>'));
        article.append(div4);
...
$(document).ready(function () {
  ...
  $(document).on('click', '.reviews h2 span', function () {
    const $span = $(this);
    if ($span.parent().next().children().length === 0) {
      $span.text('hide');
      $.get(`http://0.0.0.0:5001/api/v1/places/${$span.attr('data-id')}/reviews/`,
        function (data) {
          data.forEach(function (r) {
            $span.parent().next().append($(`<li><p>${r.text}</p></li>`));
          });
        });
    } else {
      $span.parent().next().empty();
      $span.text('show');
    }
  });
});
